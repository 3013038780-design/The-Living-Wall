...
本地联调：在 `tools/depth-lab` 用 Python 3.12 虚拟环境运行 `.venv/bin/python server.py --port 8769`，连接 Gemini 335 并校准空墙；项目根目录运行 `npm run dev`，点击主页面的“启用深度实验”。页面只接受真实相机的有效近墙区域，支持水平和垂直镜像，失联后清除输入。生产页面没有此入口或本地桥接接口。
