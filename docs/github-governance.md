...
审批人数为 0，保留常规 PR 自检后自行合并的政策；重大变更须另一人复核，由协作规则与 PR 模板落实，当前配置不自动识别变更类型。无管理员绕过例外。不强制线性历史，以保留有说明的 merge 选择。
...
预览可使用任务分支；生产默认使用已合入 `main` 且验证通过的提交。例外按 AGENTS.md 复核，记录修复如何回合主线。同一环境串行部署；紧急回退也记录实际回退产物及验证。
