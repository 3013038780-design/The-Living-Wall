...
export class Guide {
 ...
 start(mode,roi,at=Date.now()){
  this.report={version:1,started_at:new Date(at).toISOString(),source:mode==='simulation'?'simulation':'camera',mode,roi:[...roi],status:'running',steps:[],samples:[],events:[],note:'仅数值，无图像或轮廓。动作是提示与用户自报，非自动验证；轮询样本不等于独立相机帧。'};
 ...
 sample(d,at=Date.now()){
  ...
  const r=d.result||{};
  this.report.samples.push({at,step:this.index,state:r.state,age_ms:d.age_ms,valid_ratio:r.valid_ratio??null,diagnostic_valid:!!r.diagnostic_valid,foreground_count:(r.regions||[]).length,near_count:(r.near_regions||[]).length,near_gaps_mm:(r.near_regions||[]).map(v=>v.gap_mm),contact_mm:r.contact_mm??null,noise_mm:r.noise_mm??null});
 }
