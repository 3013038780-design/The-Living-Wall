        ...
        while True:
            ...
            with self.lock:
                ...
                if self.started is not None:
                    ...
                    self.message = '已连接，可以开始' if good else '相机未就绪：请在原测试台完成空墙校准'
            time.sleep(.1)
...
<button id="start" disabled>开始测试</button><p id="action">正在连接…</p><b id="count"></b><p id="stats"></p>
<canvas id="view" width="960" height="500"></canvas><p><small>圆点表示持续检测到的近墙区域，不代表已确认手掌或真实接触。编号仅用于短时跟踪。</small></p><p id="saved"></p>
<script>
